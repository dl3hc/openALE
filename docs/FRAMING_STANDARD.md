...
| Address accumulation | `AddressEncoder` (TX); RX: `collecting_remote_conclusion`, `hs_conclusion_rcvd` (ale_call_processor.cpp) — **`linked_twas_addr_`/`linked_twas_last_ms_` retired (2026-08-31, Phase 3b)**, absorbed into the FrameReassembler's FR-03 run | two ad-hoc accumulators remain (CALLING/HANDSHAKE); Phase 3c folds them onto reassembler roles |
| Role assignment | `classify()` + `WordRole` (ale_call_processor.cpp:37-114); **`FrameReassembler` (src/Protocol/Frame/, live consumer since 2026-08-31)** assigns `ParseRole` from parse position per FR-01..05, emits `AssembledFrame` events, grammar-typed | classify() migrates to reassembler roles in Phase 3c; equivalence pinned by tests/frame/unit/test_frame_reassembler.cpp (identity/role/boundary vs the Phase-0 pins) |
