...
| Role assignment | `classify()` + `WordRole` (ale_call_processor.cpp:37-114); **`FrameReassembler` (src/Protocol/Frame/, shadow, 2026-08-31)** assigns `ParseRole` from parse position per FR-01..05, emits `AssembledFrame` events, grammar-typed | classify() migrates to reassembler roles in Phase 3; equivalence pinned by tests/frame/unit/test_frame_reassembler.cpp (identity/role/boundary vs the Phase-0 pins) |
...
| Boundary decision | `handle_linked()` settle (ale_state_machine.cpp:901-911); `tick_frame_settle()` (App); `SoundingIdentityAccumulator` | **partially converged:** the reassembler's FR-01 tick lands on the same Tdrw settles (pinned); its Frame events drive decisions in Phase 3 |
| Context gating | per-state `react_*` switch (ale_call_processor.cpp:426-480); **§8 table as code (2026-08-31, Phase 3a): `context_matrix_lookup()` (include/Protocol/Control/context_matrix.h + src/Protocol/Control/context_matrix.cpp)** | table exists, transcribed cell-for-cell from §8 (tests/link/unit/test_context_matrix.cpp, ctest `ContextMatrix`, every state×type cell + the F-08 ✓* footnote cells + UNTAGGED/ERROR fail-safe defaults) — **nothing consults it yet**; `react_*` still decides directly. Phase 3b wires the LINKED termination decision through it first |
