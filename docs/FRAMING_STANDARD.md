...
| Address accumulation | TX: `AddressEncoder`. RX: **`collecting_remote_conclusion` retired (2026-08-31, Phase 3c)** — classify()'s CALLING gate now reads `frame_reassembler_.candidate()->complete && last_role()==ADDRESS_EXTENSION` directly (FR-03 parse position); `linked_twas_addr_`/`linked_twas_last_ms_` retired in Phase 3b. `hs_conclusion_rcvd`/`hs_ack_tis_rcvd` (HANDSHAKE) deliberately kept — see the note below | HANDSHAKE's two flags are the one remaining ad-hoc accumulator, and are staying by design, not by omission |
| Role assignment | `classify()` + `WordRole` (ale_call_processor.cpp:37-114); **`FrameReassembler` (src/Protocol/Frame/, live consumer since 2026-08-31)** assigns `ParseRole` from parse position per FR-01..05, emits `AssembledFrame` events, grammar-typed | CALLING's gate migrated (Phase 3c, above); HANDSHAKE's two flags intentionally remain (see note below). Equivalence pinned by tests/frame/unit/test_frame_reassembler.cpp + tests/link/unit/test_rx_characterization.cpp |
...
| Boundary decision | **converged for LINKED termination (2026-08-31, Phase 3b):** `ALEStateMachine::handle_completed_frame_()` (ale_state_machine.cpp), called from `update()`'s FrameReassembler drain for every completed frame, ahead of the per-state `handle_*()` — same Tdrw settle window the retired accumulator used (pinned by ForeignTwasLinkGuard, unmodified). `tick_frame_settle()` (App); `SoundingIdentityAccumulator` still separate | remaining states/frame types route through `handle_completed_frame_()` in Phase 3c/3d |
| Context gating | per-state `react_*` switch (ale_call_processor.cpp); **§8 table as code (2026-08-31, Phase 3a): `context_matrix_lookup()` (include/Protocol/Control/context_matrix.h + src/Protocol/Control/context_matrix.cpp)**, `ContextMatrix` ctest | table exists, transcribed cell-for-cell from §8 — **`handle_completed_frame_()` does not query it yet** (see the F-05/F-06 note below on why the LINKED termination decision cannot key on the coarse table); Phase 3c/3d route the remaining states' decisions through it |

**F-05/F-06 grammar-sharing note (found during Phase 3b):** `FrameReassembler::assign_frame_type_()` never emits `FrameType::F_TERMINATION` or `F_ACK` — every `TO×2 addr + conclusion` candidate (§6 note: F-03/F-04/F-05 share one grammar) types as `F_RESPONSE`, and a conclusion caught without its leading `TO×2` (a real possibility — the pinned termination tests never send one) types as `F_SOUND`. So a genuine LINKED-peer termination and a foreign station's sound are not just contextually ambiguous, they can be *grammar-type-identical* (both `F_SOUND`). §8's separate F-05 (term.) / F-06 (sound) **columns are the catalog's conceptual split — not two RX-distinguishable `FrameType` values.** `handle_completed_frame_()` therefore keys the LINKED decision on `conclusion_is_twas` directly (any completed frame whose conclusion is TWAS, regardless of `f.type`) and applies the F-05 row's own rule — exact full-address match vs `active_call_to` — as the disambiguator; a mismatch reduces to exactly what the matrix's LINKED×F_SOUND=OBSERVE cell already prescribes (discard, FR-08). This is the one decision point Phase 3c/3d should NOT try to force through a flat `context_matrix_lookup(state, f.type) == ACT` gate — document it, don't "fix" it away.

**`hs_conclusion_rcvd`/`hs_ack_tis_rcvd` — intentionally remaining (decided during Phase 3c):** unlike `collecting_remote_conclusion` (a pure classify()-internal proxy, now retired), these two are **public API**
(`ALEStateMachine::is_hs_conclusion_rcvd()`, consumed by `App/ale_controller.cpp`)
with **pinned cross-phase persistence** that has nothing to do with parse
position: `tests/word/unit/test_ale_calling.cpp` deliberately asserts
`hs_conclusion_rcvd` STAYS true after a HANDSHAKE is aborted back to
SCANNING, clearing only on the *next* HANDSHAKE entry — a fact about SM
lifecycle the FrameReassembler's candidate (which just closes/reopens with
the word stream) cannot express or reproduce. Their `collecting`-gate use
during WAIT_ACK is also timing-only (arms `hs_ack_to_ms`/`hs_tlww_start_ms`;
no identity text depends on it, per the WAIT_ACK case comment in
`react_handshake_`), so there is no accumulation logic left to simplify —
migrating them would trade a real, tested behavior for no simplification.
Left as-is; do not migrate without an explicit owner decision to change the
pinned stale-carryover semantics first.

### 10.1 OF-0 audit — word-level survivors (Phase 3d)

Every direct `process_event()`/state-changing call inside `ale_call_processor.cpp`'s
react paths that fires from a single word, not a frame boundary (grep:
`process_event(ALEEvent::` in that file, cross-checked against `on_invalid_word_`,
`detect_incoming_call_`, and every `react_*`):
